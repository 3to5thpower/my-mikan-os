...
void Log(LogLevel level, const TransferEventTRB& trb) {
    if (trb.bits.event_data) {
        Log(level,
            "Transfer (value %08lx) completed: %s, residual length %d, slot "
            "%d, ep addr %d\n",
...
Error Device::ControlIn(EndpointID ep_id, SetupData setup_data, void* buf,
    ...
    if (auto err =
            usb::Device::ControlIn(ep_id, setup_data, buf, len, issuer)) {
        return err;
    }
...
Error Device::ControlOut(EndpointID ep_id, SetupData setup_data,
    ...
    if (auto err =
            usb::Device::ControlOut(ep_id, setup_data, buf, len, issuer)) {
        return err;
    }
